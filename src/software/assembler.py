## Criado por Samuel Gadiel de Ávila https://github.com/SamuelGadiel/nand2tetris/blob/master/Week6/hackassembler.py
        ...
        if leading_character == "@":
            ...
            if "=" not in command_str:
                if ";" not in command_str:
                    ...
                    command.computation, command.jump = list(
                        map(str.strip, command_str.split(";"))
                    )
            else:
                if ";" not in command_str:
                    command.destination, command.computation = list(
                        map(str.strip, command_str.split("="))
                    )
                else:
